...
export function WeeklyLine({ points }: { points: { week: string; total: number }[] }) {
    ...
    <div className="h-64">
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 8, right: 10, left: 0, bottom: 8 }}>
          ...
          <XAxis dataKey="week" tick={{ fill: 'rgba(237,241,255,0.70)', fontSize: 12 }} />
          ...
          />
          <Line type="monotone" dataKey="total" stroke="#ff9100" strokeWidth={2.5} dot={false} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  )
